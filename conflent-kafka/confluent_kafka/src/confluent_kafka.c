...
#define MAX_HTTP_BODY_LEN   1024000
...
typedef struct {
    uint64_t    sig_id;
    uint64_t    sig_name_len;
    uint64_t    url_len;
    uint64_t    http_json_len;
    uint64_t    request_body_len;
    uint64_t    response_body_len;
    char        data[0];
}kafka_msg_t;
...
static PyObject *Message_sig_id(Message *self, PyObject *ignore) {
    return PyLong_FromLongLong(self->sig_id);
}

static PyObject *Message_sig_name(Message *self, PyObject *ignore) {
	if (self->sig_name) {
		Py_INCREF(self->sig_name);
...
static PyObject *Message_http_header (Message *self, PyObject *ignore) {
	if (self->http_header) {
		Py_INCREF(self->http_header);
		return self->http_header;
	} else
		Py_RETURN_NONE;
}

static PyObject *Message_request_body_len (Message *self, PyObject *ignore) {
    return PyLong_FromLongLong(self->request_body_len);
}


static PyObject *Message_request_body (Message *self, PyObject *ignore) {
	if (self->request_body) {
		Py_INCREF(self->request_body);
...
static PyObject *Message_response_body_len (Message *self, PyObject *ignore) {
    return PyLong_FromLongLong(self->response_body_len);
}

static PyObject *Message_response_body (Message *self, PyObject *ignore) {
	if (self->response_body) {
		Py_INCREF(self->response_body);
		return self->response_body;
	} else
		Py_RETURN_NONE;
}

static PyObject *Message_url_len (Message *self, PyObject *ignore) {
    return PyLong_FromLongLong(self->url_len);
}

static PyObject *Message_url (Message *self, PyObject *ignore) {
	if (self->url) {
		Py_INCREF(self->url);
		return self->url;
	} else
...
static PyMethodDef Message_methods[] = {
    ...
    { "sig_id", (PyCFunction)Message_sig_id, METH_NOARGS,
	  "  :returns: signature id.\n"
	  "  :rtype: int or None\n"
	  "\n"
	},

    { "sig_name", (PyCFunction)Message_sig_name, METH_NOARGS,
	  "  :returns: signature name.\n"
	  "  :rtype: str|bytes or None\n"
	  "\n"
	},

    { "http_header", (PyCFunction)Message_http_header, METH_NOARGS,
	  "  :returns: http_header or None if not available.\n"
	  "  :rtype: str|bytes or None\n"
	  "\n"
	},

    { "request_body_len", (PyCFunction)Message_request_body_len, METH_NOARGS,
	  "  :returns: request body len.\n"
	  "  :rtype: int or None\n"
	  "\n"
	},

    { "request_body", (PyCFunction)Message_request_body, METH_NOARGS,
	  "  :returns: request body.\n"
	  "  :rtype: str|bytes or None\n"
	  "\n"
	},

    { "response_body_len", (PyCFunction)Message_response_body_len, METH_NOARGS,
	  "  :returns: response body len.\n"
	  "  :rtype: int or None\n"
	  "\n"
	},

    { "response_body", (PyCFunction)Message_response_body, METH_NOARGS,
	  "  :returns: response body.\n"
	  "  :rtype: str|bytes or None\n"
	  "\n"
	},

    { "url_len", (PyCFunction)Message_url_len, METH_NOARGS,
	  "  :returns: url len.\n"
	  "  :rtype: int or None\n"
	  "\n"
	},

    { "url", (PyCFunction)Message_url, METH_NOARGS,
	  "  :returns: url.\n"
	  "  :rtype: str|bytes or None\n"
	  "\n"
	},
...
static int Message_clear (Message *self) {
    ...
    if (self->sig_name) {
		Py_DECREF(self->sig_name);
		self->sig_name = NULL;
	}
    if (self->http_header) {
		Py_DECREF(self->http_header);
...
static int Message_traverse (Message *self,
	...
	if (self->value)
		Py_VISIT(self->value);
    if (self->sig_name)
		Py_VISIT(self->sig_name);
    if (self->http_header)
		Py_VISIT(self->http_header);
    if (self->request_body)
		Py_VISIT(self->request_body);
    if (self->response_body)
...
static int message_check(const rd_kafka_message_t *rkm)
{
    kafka_msg_t *msg;
    uint64_t    msg_len;

    if (rkm->len) {
        msg = (kafka_msg_t *)rkm->payload;

        if (msg->request_body_len > MAX_HTTP_BODY_LEN) {
            printf("Request_body_len error: %lu\n\r", msg->request_body_len);
            return -1;
        }

        if (msg->response_body_len > MAX_HTTP_BODY_LEN) {
            printf("Response_body_len error: %lu\n\r", msg->response_body_len);
            return -1;
        }

        msg_len = sizeof(kafka_msg_t) + msg->sig_name_len + msg->url_len + msg->http_json_len + msg->request_body_len + msg->response_body_len;
        if (rkm->len != msg_len+1) {
            printf("msg error : rkm_len %lu, msg_len %lu, kafka_msg_t %lu, sig_name_len %lu, url_len %lu, http_head_len %lu, request_body_len %lu, response_body_len %lu\n\r",
                rkm->len, msg_len, sizeof(kafka_msg_t), msg->sig_name_len, msg->url_len, msg->http_json_len, msg->request_body_len, msg->response_body_len);
            return -1;
        }
...
PyObject *Message_new0 (const Handle *handle, const rd_kafka_message_t *rkm)
{
	...
	if (rkm->rkt) {
		self->topic = cfl_PyUnistr(_FromString(rd_kafka_topic_name(rkm->rkt)));
	}

    if(message_check(rkm) != 0) {
        self->error = KafkaError_new_or_None(RD_KAFKA_RESP_ERR_UNKNOWN, "kafka message error");
        return (PyObject *)self;
    }

	if (rkm->len) {
		self->value = cfl_PyBin(_FromStringAndSize(rkm->payload, rkm->len));
        p = rkm->payload;
        msg = (kafka_msg_t *)p;

        p += sizeof(kafka_msg_t);

        self->sig_id = msg->sig_id;
        self->url_len = msg->url_len;
        self->request_body_len = msg->request_body_len;
        self->response_body_len = msg->response_body_len;


        self->sig_name = cfl_PyBin(_FromStringAndSize(p, msg->sig_name_len));
        p += msg->sig_name_len;

        if (msg->url_len) {
            self->url = cfl_PyBin(_FromStringAndSize(p, msg->url_len));
            p += msg->url_len;
        } else {
            self->url = NULL;
        }

        if (msg->http_json_len) {
            self->http_header = cfl_PyBin(_FromStringAndSize(p, msg->http_json_len));
            p += msg->http_json_len;
        } else {
            self->headers = NULL;
        }

        if (msg->request_body_len) {
            self->request_body = cfl_PyBin(_FromStringAndSize(p, msg->request_body_len));
            p += msg->request_body_len;
        } else {
            self->request_body = NULL;
        }

        if (msg->response_body_len) {
            self->response_body = cfl_PyBin(_FromStringAndSize(p, msg->response_body_len));
            p += msg->response_body_len;
        } else {
            self->response_body = NULL;
        }
	}
	...
	if (rkm->key) {
		self->key = cfl_PyBin(_FromStringAndSize(rkm->key, rkm->key_len));
	}
...
static rd_kafka_headers_t *py_headers_dict_to_c (PyObject *hdrs) {
        ...
        while (PyDict_Next(hdrs, &pos, &ko, &vo)) {
                ...
                if (vo != Py_None) {
                        ...
                        if (PyBytes_AsStringAndSize(vo, (char **)&v,
                                                     &vsize) == -1) {
                                Py_DECREF(ks);
                                rd_kafka_headers_destroy(rd_headers);
                                return NULL;
